...
// Use object destructuring with the rest parameter to isolate the `first` and `second` locations from the `locations` object, capturing the rest in a variable called `remaining`. 
// Print the key locations.
const { first, second, ...location } = locations
console.log('Task 2 -> ', first, second)
...
// TODO: To open the Mysterious Door, a sequence is required, which might be incomplete. Use object destructuring to assign default values to ensure the door opens even if part of the code is missing. 
...
const { upper = 'Tortuga', middle = upper, lower = 'Rata', ...remaining } = doorCode // ?????????????????
console.log('Task 3 -> ', upper, middle, lower)
...
console.log('Task 4 -> ', translation)
...
console.log('Task 5 -> ', firstEl, secondEl)
...
console.log('Task 6 -> ', firstStone, sixthStone)
...
console.log('Task 7 -> ', `visible: ${visible} / hidden: ${hidden}`)
...
console.log('Task 8 -> ', revealPath(path))

/* Task 9: The Scroll of Defaults */
// TODO: Destructo finds an ancient scroll with a potion recipe, but some ingredients are missing. Write a function `mixPotion` that uses defaults "Water" and "Fireflower" for `ingredient1` and `ingredient2` if they are not specified and print those mixings. 
// The function takes an object with these optional properties.
function mixPotion({ ingredient1 = "Water", ingredient2 = 'Fireflower', ...other }) {
	return {
		ingredient1,
		ingredient2,
		...other
...
let potion = {
	ingredient3: 'Frogs Breath',
	ingredient4: 'Garbage',
}

console.log('Task 9 -> Potion:', mixPotion(potion))
...
function castSpell([ spellOne, spellTwo ]) {
	return [ spellOne, spellTwo ]
}

const spell = ['Fire', 'Ice', 'Mud', 'Blast', 'Wind']
console.log('Task 10 -> ', castSpell(spell))
...
console.log('Task 12 -> ', `${stoneA} amd ${stoneB}`)
